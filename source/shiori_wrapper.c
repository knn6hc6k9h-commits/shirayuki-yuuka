...
static void init_feature(void){int i;for(i=0;i<48;++i)g_feature[i]=0;g_feature[0]='Y';g_feature[1]='U';g_feature[2]='K';g_feature[3]='F';p32(g_feature+4,4);g_has_last_seen=0;}
...
static void save_feature_data(void){
  ...
  p32(g_feature+4,4);build_path(path,(int)sizeof(path),g_dir,"yuuka_features.dat");
...
static HGLOBAL birthday_settings(long* outlen){
  char b[1024];int p=0;unsigned long m=u32(g_feature+36),d=u32(g_feature+40);b[0]=0;
...
__declspec(dllexport) HGLOBAL __cdecl request(HGLOBAL h,long* lenp){
  int inlen=(lenp?(int)*lenp:0);const char* event=0;unsigned long secs,count,slot,m,d,lastyear;int affection,bm,bd;HGLOBAL r;long rn=0;SYSTEMTIME now;char input[64];
